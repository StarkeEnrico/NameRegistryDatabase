CREATE PROCEDURE dbo.usp_RegistrationLastServiceSequence (@ServiceID int, @ServiceRegionID int, @ProviderAccountId int, @LastSequence int OUTPUT)
AS 
/*
	.SYNOPSIS
	Determine the last sequence number based of a service.
...
*/
BEGIN
	DECLARE @maxSequenceLength int
	SET @maxSequenceLength = 5
			...
			MAX(
				CAST(
					SUBSTRING(
						  r.[Name]
						, PatIndex(''%[0-9]%'', r.[Name])
						, ' + CAST(@maxSequenceLength AS nvarchar(10)) + 
					')
					AS int)
				)
			,0
	...
	SELECT @LastSequence = Seq FROM @lastSeq
END