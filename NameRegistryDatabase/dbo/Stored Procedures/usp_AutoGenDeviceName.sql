...
CREATE PROCEDURE [dbo].[usp_AutoGenDeviceName]
...
IF @reuseFreeNumbers = 0
	BEGIN
		SELECT @seq = RIGHT('00000' + CONVERT(varchar(5), MAX(SUBSTRING(dev.dev_name, PatIndex('%[0-9]%', dev.dev_name), @maxlen))	+ 1), @maxlen)
				  FROM tbl_Device AS dev
				  WHERE SUBSTRING(dev.dev_name,4,LEN(@DeviceTypeShortName)) = @DeviceTypeShortName
					...
					BEGIN
						...
						WHERE seq NOT IN
							(
							  ...
							  SELECT SUBSTRING(dev.dev_name, PatIndex('%[0-9]%', dev.dev_name), @maxlen)
							  FROM tbl_Device AS dev
							  WHERE SUBSTRING(dev.dev_name,4,LEN(@DeviceTypeShortName)) = @DeviceTypeShortName
							)
						GROUP BY seq
